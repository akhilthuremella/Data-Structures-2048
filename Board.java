...
public class Board {
    ...
    public void updateOpenSpaces() {
        ...
        for (int i = 0; i<gameBoard.length; i++){
            for (int k = 0; k<gameBoard[0].length; k++){
                if (gameBoard[i][k]==0){
                    openSpaces.add(new BoardSpot(i,k));
                }
            }
        }
    }
    ...
    public void addRandomTile() {
        ...
        BoardSpot randomtile = openSpaces.get(t);
        
        if (v<0.1){
            gameBoard[randomtile.getRow()][randomtile.getCol()]=4;
        }
        else {
            gameBoard[randomtile.getRow()][randomtile.getCol()]=2;
        }
    ...
    public void swipeLeft() {
        for (int i=0; i<gameBoard.length; i++){
            ...
            for (int k = 0; k<gameBoard[i].length; k++){
                if (gameBoard[i][k] != 0){
                    gameBoard[i][ind++] = gameBoard[i][k];

                }
                
            }
            for (int k = ind; k<gameBoard[i].length; k++){
                gameBoard[i][k]=0;
            }
        }
    }
    ...
    public void mergeLeft() {
        for (int i = 0; i<gameBoard.length; i++){
            for (int k=0; k<gameBoard[i].length-1; k++){
    ...
    public void transpose() {
        ...
        for (int i = 0; i<gameBoard.length; i++){
            for (int k = 0; k<i; k++){
                transpose = gameBoard[i][k];
                gameBoard [i][k] = gameBoard [k][i];
                gameBoard [k][i]= transpose;
            }
        }
    }
    ...
    public void flipRows() {
        ...
        for (int i = 0 ; i<gameBoard.length; i++) {
            ...
            while (left<right){
                flip = gameBoard [i][left];
                gameBoard [i][left]= gameBoard[i][right];
                gameBoard[i][right]= flip;
                left ++;
                right--;
            }
        }   
